...
public class NewMultiphysicsCloth : MonoBehaviour
{
    ...
    public void BuildCloth(XPBDSolver solver)
    {
        ...
        geoVerts = new Vertex[numParticlesX, numParticlesY];
    ...
    public void RenderCloth()
    {
        ...
        for (int i = 0; i < numParticlesX; i++)
            for (int j = 0; j < numParticlesY; j++)
            {
                ...
                geoVerts[i, j].Position = transform.InverseTransformPoint(worldPos);
            }

        var updated = geom.BuildUnityMesh();
        var m = meshFilter.mesh;
        m.vertices = updated.vertices;
        m.triangles = updated.triangles;
